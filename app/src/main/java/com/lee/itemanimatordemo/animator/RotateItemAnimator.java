...
public class RotateItemAnimator extends BaseItemAnimator {
    ...
    void animateAddImpl(final RecyclerView.ViewHolder holder) {
        ...
        final int quan = (int) (getAddDuration() / 500);
        animation.alpha(1).rotation(quan * 360).setDuration(getAddDuration())
                .setListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationStart(Animator animator) {
                        Log.i("animation", "Rotate Add Start"+quan);
                        dispatchAddStarting(holder);
                    }

                    @Override
                    public void onAnimationCancel(Animator animator) {
                        Log.i("animation", "Rotate Add Cancel");
                        view.setRotation(0);
                        view.setAlpha(1);
                    }
                    ...
                    @Override
                    public void onAnimationEnd(Animator animator) {
                        Log.i("animation", "Rotate Add End");
                        animation.setListener(null);
    ...
    void animateRemoveImpl(final RecyclerView.ViewHolder holder) {
        ...
        final int quan = (int) (getRemoveDuration() / 500);

        animation.setDuration(getRemoveDuration()).alpha(0).rotation(quan * 360).setListener(
                new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationStart(Animator animator) {
                        Log.i("animation", "Rotate Remove Start" + quan);
                        dispatchRemoveStarting(holder);
                    }

                    @Override
                    public void onAnimationEnd(Animator animator) {
                        Log.i("animation", "Rotate Remove End");
                        animation.setListener(null);
                        view.setAlpha(1);
                        view.setRotation(0);
